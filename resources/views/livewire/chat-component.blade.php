        ...
        <div style="overscroll-behavior: none;">
            <div class="fixed w-full bg-green-400 h-16 pt-2 text-white flex justify-between shadow-md"
                ...
                <div class="my-3 text-green-100 font-bold text-lg tracking-wide">
                    {{ $user->name ? $user->name : 'No user name' }}</div>
                <!-- 3 dots -->
            ...
            <div class="mt-20 mb-16">
                @foreach ($messages as $item)
                    @if ($item['sender'] != auth()->user()->name)
                        <div class="clearfix w-4/4 ">
                            <div class="bg-gray-300  mx-4 my-2 p-2 rounded-lg inline-block"><b>{{$item['sender']}} :</b>{{$item['message']}}</div>
                        </div>
                    @else
                        <div class="clearfix w-4/4">
                            <div class="bg-green-300 float-right  mx-4 my-2 p-2 rounded-lg  inline-block clearfix"><b>You :</b>{{$item['message']}}</div>
                        </div>
                    @endif
            ...
            <div class="fixed w-full flex justify-between bg-green-100" style="bottom: 0px;">
                ...
                <button class="m-2" type="submit" style="outline: none;">
                    <svg class="svg-inline--fa text-green-400 fa-paper-plane fa-w-16 w-12 h-12 py-2 mr-2"
                        aria-hidden=o"true" focusable="false" data-prefix="fas" data-icon="paper-plane" role="img"
